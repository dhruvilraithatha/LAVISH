...
function getlastActiveTime($username){
    $json3 = file_get_contents(dirname(__FILE__).'/includes/json/online-status.json');
...
function updatelastActiveTime(){
    ...
    file_put_contents(dirname(__FILE__).'/includes/json/online-status.json', $json);
}
...
function searchUser($con,$config){
    ...
    if($_POST)
    {
        ...
        $row1[$TFusername];
        $sesuserpic = "../FRIEND_FEED/".$row1[$TFphoto];

        if($sesuserpic == "../FRIEND_FEED/images/")
        ...
        while($row=mysqli_fetch_array($sql_res))
        {
            ...
            $picname = $row[$TFphoto];
            if($picname == "")
                $picname = "../FRIEND_FEED/images/avatar_default.png";
            ...
            <li class="person chatboxhead" id="chatbox1_<?php echo $username ?>" data-chat="person_<?php echo $id ?>" href="javascript:void(0);" onclick="javascript:chatWith('<?php echo $username ?>','<?php echo $id ?>','<?php echo $sesuserpic; ?>','<?php echo $onofst ?>')">
                <a href="javascript:void(0)">
                    <span class="userimage profile-picture min-profile-picture"><img src="../FRIEND_FEED/<?php echo $picname; ?>" alt="<?php echo $username ?>" class="bg-theme"></span>
            <span>
        ...
        <div class="user-bg">
            <div class="overlay-box">
                <div class="user-content"> <a href="javascript:void(0)">
                        <img class="thumb-lg img-circle" src="../FRIEND_FEED/<?php echo $picname; ?>" alt="<?php echo $username ?>"></a>
...
function chatfrindList($con,$config) {
    ...
    while ($row = mysqli_fetch_array($result)) {
        ...
        <li class="person chatboxhead" id="chatbox1_<?php echo $username ?>" data-chat="person_<?php echo $id ?>" href="javascript:void(0)" onclick="javascript:chatWith('<?php echo $username ?>','<?php echo $id ?>','<?php echo $sesuserpic; ?>','<?php echo $onofst ?>')">
            <a href="javascript:void(0)">
                <span class="userimage profile-picture min-profile-picture"><img src="<?php echo $picname; ?>" alt="<?php echo $username ?>" class="avatar-image is-loaded bg-theme" width="100%"></span>
                <span>
...
function get_all_msg($con,$config) {
    ...
    while ($chat = mysqli_fetch_array($query)) {
        ...
        {
            $picname = "../FRIEND_FEED/".$info[$TFPicname];
            //$status = $info['online'];
        }
        ...
        {
            $picname2 = "../FRIEND_FEED/".$info4[$TFPicname];
        }


        if($picname == "../FRIEND_FEED/")
            $picname = "../FRIEND_FEED/images/avatar_default.png";

        if($picname2 == "../FRIEND_FEED/")
            $picname2 = "../FRIEND_FEED/images/avatar_default.png";
...
EOD;
    ...
    $sql = "insert into `" . $config['db']['pre'] . "messages` (from_uname,to_uname,from_id,to_id,message_content,message_type,message_date) values ('" . $from . "', '" . $to . "','" . $from_id . "','" . $to_id . "','" . addslashes($message) . "','text','".$GLOBALS['timenow']."')";
...
function typingStatus() {
    ...
    file_put_contents(dirname(__FILE__).'/includes/json/typing-status.json', $json);*/
