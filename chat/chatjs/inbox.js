...
function updateSeenmsg(chatuser) {
    $.post(siteurl+"chat.php?action=updateSeenmsg", {chatuser: chatuser} , function(data){
    });
...
function chatWith(chatuser,toid,img,status) {
    ...
    if ($("#pane-intro").css('visibility') == 'visible')
    {
        ...
        $(".wchat .chat-left-aside .open-panel").css({'display':'none'});
    }
    TOusername = chatuser;
    clearTimeout(chkSeenInterval);
    clearTimeout(intervalChatHeart);
    intervalChatHeart = setTimeout('chatHeartbeat(TOusername);',chatHeartbeatTime);
    scrollDown();
...
function createChatBox(chatboxtitle,toid,img,status,minimizeChatBox) {
    ...
    get_all_msg(siteurl+"chat.php?page=1&action=get_all_msg&client="+chatboxtitle);
          ...
          $.each(data.items, function(i,item){
			if (item)	{ // fix strange ie bug
				...
				} else
                {
                    ...
                    if (item.u == 2) {
                        $("#chatbox_"+chatboxtitle).prepend('<div class="col-xs-12 p-b-10"><div class="chat-image  profile-picture max-profile-picture"> <img alt="'+item.sender+'" src="'+img+'" class="bg-theme"> </div><div class="chat-body"><div class="chat-text"><h4>'+item.sender+'</h4><p>'+message_content+'</p><b>'+item.time+'</b> </div></div></div>');
					} else
                    {
                        if(seen == 1){
...
function startChatSession(){
                ...
                if (item)	{ // fix strange ie bug
                    ...
                    if (item.s == 2) {
                        ...
                        if (item.u == 2) {
                            ...
                            //$("#chatbox_"+chatboxtitle+" .chatboxcontent").append('<div class="chatboxmessage direct-chat-msg"><div class="direct-chat-info clearfix"><span class="direct-chat-name pull-left">'+item.f+'</span></div><img class="direct-chat-img" src="'../FRIEND_FEED/'+item.p+'" alt="message user image"><span class="direct-chat-text">'+item.m+'</span></div>');
                        } else {

                            //$("#chatbox_"+chatboxtitle+" .chatboxcontent").append('<div class="chatboxmessage direct-chat-msg right"><div class="direct-chat-info clearfix"><span class="direct-chat-name pull-right">'+item.f+'</span></div><img class="direct-chat-img" src="'../FRIEND_FEED/'+img+'" alt="message user image"><span class="direct-chat-text">'+item.m+'</span></div>');
                        }
                    }
                }
            });
    ...
    intervalChatHeart = setTimeout('chatHeartbeat("null");',chatHeartbeatTime);
    scrollDown();
}
    ...
    if((event.keyCode == 13 && event.shiftKey == 0) || (send == 1))  {
        ...
        if (message != '') {
            $.post(siteurl+"chat.php?action=sendchat", {to: chatboxtitle, toid: toid, message: message} , function(data){
                ...
                '<div class="chat-image  profile-picture max-profile-picture">' +
                '<img alt="'+username+'" src="../FRIEND_FEED/'+img+'">' +
                '</div>' +
                '<div class="chat-body">' +
                '<div class="chat-text">' +
                '<h4>'+username+'</h4>' +
...
function chatHeartbeat(TOusername){
	...
	$.ajax({
	  url: siteurl+"chat.php?action=chatheartbeat&fromuname="+TOusername,
	  cache: false,
	  dataType: "json",
	  success: function(data) {
			...
			if (item)	{ // fix strange ie bug
                ...
                if(typeof item.f != "undefined")
                {
                    ...
                    } else if(cLenth != 0)
                    {
                        ...
                        '<div class="chat-image  profile-picture max-profile-picture">' +
                        '<img alt="' + chatboxtitle + '" src="../FRIEND_FEED/' + fromPic + '" class="bg-theme">' +
                        '</div>' +
                        '<div class="chat-body">' +
                        '<div class="chat-text">' +
                        '<h4>' + chatboxtitle + '</h4>' +
...
function chatfrindList(){
    $.ajax({
        url: siteurl + "chat.php?action=chatfrindList",
        cache: false,
        type: "POST",
        success: function (data) {
...
function checkMsgSeen(id,chatuser){
    $.ajax({
        url: siteurl + "chat.php?action=checkMsgSeen&msgid="+id+"&uname="+chatuser,
        cache: false,
        type: "POST",
        success: function (data) {
            if(data == 1)
...
function userProfile(username){
    $.ajax({
        url: siteurl + "chat.php?action=userProfile&uname="+username,
        cache: false,
        type: "POST",
        success: function (data) {
...
function lastseen(username){
    $.ajax({
        url: siteurl + "chat.php?action=lastseen&uname="+username,
        cache: false,
        type: "POST",
        success: function (data) {
...
function savechat(mail){
    ...
    $.ajax({
        url: siteurl + "chat.php?action=savechat&uname="+username+"&mail="+mail,
        cache: false,
        type: "POST",
        success: function (data) {
            if(mail == false){
...
$(document).ready(function(){
    ...
    $('.search_bg').on('keyup', function(){
        ...
        else
        {
            ...
            $.ajax({
                type: "POST",
                url: siteurl+"chat.php?action=searchUser",
                data: dataString,
                cache: false,
                success: function(data)
                {
